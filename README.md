...
- 战斗关键字：连击/先攻/贯通/穿刺/远程/不屈/迅捷/屏障；全量伤害时点批次管线；关键字三类持久性（一次性/持续/永久，多重集）；作用域战斗伤害免疫
- 攻击后到期强化（attack_buffs，残心可保留）；法术觉醒替换（能力换为觉醒牌 abilities 块，气绝/复活保留；觉醒·兵俑护甲保留）
- 增强装配管线：卡牌触发器（triggers 全库扫描）、打出装配快照（persistent store → 实例 mods）、enhance 数值叠加、卡牌光环（card_auras：瞬发/不耗鬼火）、战斗绑定一次性触发（temp_grants）
- 倒计时系统（锚点版）：形态倒计时结附获得、己方回合开始 -1、归零重置并执行倒计时效果、离场/气绝移除；形态能力块（结附期间生效）；投射/鼓舞（出击加成）/直接消灭/随机生成（generate）
- 响应插入使用：响应战斗牌改为移入战斗区、加成持续至被插入的战斗后；响应形态牌立即结附；choose 目标自动选事件被攻击者；延迟触发（"会"）、伤害上限（"森罗之阵"）、激怒与尘缚之阵战斗区锁定、进场时形态效果与动态数值（尘刀快照/古尘之壁/援护）
- 正式数据（db/cards、db/shikigami YAML）：4 式神 32 卡全录（白狼/兵俑/妖刀姬/一目连，数据源 thoughts.txt）
