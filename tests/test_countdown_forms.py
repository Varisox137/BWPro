...
def _po(db, token=True):
    return _form(db, PO, countdown=2, token=token, cd_steps=[
...
def _play(g, pi, cid) -> None:
    g.apply({"op": "play_card", "uid": give(g, pi, cid).uid})
...
def test_po_countdown_cycle_projectile(db, make_game):
    ...
    _move(g, 1, 0)               # B 式神驻留战斗区（其回合开始才会退回）
...
def test_shi_boost_consumed_by_assault_only(db, make_game):
    ...
    _play(g, 0, combat_cid)      # 战斗牌直击牌手：不消耗出击加成
    assert pa.assault_boosts == [{"power": 3, "shield": 3}]
...
def test_long_target_count_scales(db, make_game):
    ...
    _play(g, 0, cid)
    s.countdown = 1
    _pass(g, 2)                  # 第 1 次触发：1 目标 6 点
    assert s.form.mods["count"] == 1
    assert total_damage(pb) == 6
    s.countdown = 1
    _pass(g, 2)                  # 第 2 次触发：2 目标各 6 点（无放回）
