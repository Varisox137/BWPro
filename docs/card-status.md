...
| 51 灵矢贯虹 | ✅ | 协战子选项（白狼侧战斗牌）：三步齐备——法术强化力量再授予（reapply_attack_buff_power：起弓/离/无我 attack_buffs 力量部分合计再授予，仅力量）、羁绊 1 萤草形态进场效果再触发（trigger_form_enter，未结附空操作）、羁绊 2 鼓舞消耗转化（维护者答复(3)） |
...
| 01 吸取 | ✅ | 投射 2（"造成2点伤害"按投射定案，raw 无限定词——出入 11）+ 鼓舞 2 护甲 |
...
| 51 森佑灵引 | ✅ | 协战子选项（萤草侧）：search_deck card_type=form + max_level="target"（不高于目标式神等级）+ direct_play_power_ge=4（目标存活且力量≥4改为直接使用——不耗火 play_from=deck）+ shuffle=false（raw 无"然后洗牌库"句——出入 12）；羁绊白狼获得[庇佑] |
...
| 08 火吻之蛇 | ✅ | stat_aura enemy_fragile_power 动态光环（敌方有破甲式神降等量力量）+ "攻击时获得[先攻]"（on_before_assault 授予，先攻快照重读赶上本场判定）+ 敌方回合开始全体敌方角色 1 破甲（回合开始破甲清除先于 on_turn_start，敌方破甲每半回合重置为 -1——出入 15） |
...
| 07 万象之书 | ✅ | [瞬发]；generate shikigami=friendly_others——出战队列其他己方式神（含 0 级/气绝，定案出入 13）各随机 1 张入手 |
...
11. **吸取**（萤草 01）：raw"造成2点伤害"无目标限定词，按投射实现（敌方战斗区优先、
    空则打敌方牌手）——第十四阶段定案。
12. **森佑灵引不洗牌库**：raw 无"然后洗牌库"句（区别于花信风），search_deck 增加
    shuffle 参数（缺省 true 沿用花信风语义），本卡置 false——第十四阶段定案。
13. **万象之书"其他己方式神"**：按出战队列全体取池（含 0 级未在场与已气绝式神），
    每位各随机生成一张其卡牌——第十四阶段定案。
14. **庇佑与屏障/护甲的结算顺序**：屏障（批次 3）与护甲吸收先于庇佑判定（扣减生命前）——
    被护甲完全吸收或屏障归零的伤害不消耗庇佑；庇佑仅抵消敌方来源的非战斗伤害。
15. **火吻之蛇的敌方回合开始破甲**：回合开始阶段的破甲清除先于 on_turn_start 能力触发，
    故无觉醒·清姬时敌方破甲每半回合重置为 -1（清除后由火吻之蛇重新赋予 1）。
16. **爆牌统一路径**：手牌上限检查落在 move_card——抽牌、生成置入手牌、调度换牌等所有
    进手路径共用同一爆牌流程（超出 hand_cap 的牌转而置入墓地）——第十四阶段定案。
17. **夺命变后与墨笔夺魂**：destroy 支持牌手目标（消灭牌手 = 直接获胜，走牌手气绝判负
