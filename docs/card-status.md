...
| 99 冰墙 | ✅ | 召唤物 0/2（kind=summon，no_attack）；[眩晕]对其造成战斗伤害的式神 |
...
| 08 觉醒·番茄 | ✅ | 20200227：+3/+3；去"随机 2 张战斗牌置手"（现代版效果）；永久变形 transform permanent=True（untransform 跳过、气绝前2 不还原）+ owner_combat 变形物用牌白名单（番茄可用跳跳妹妹的战斗牌）；replace_cards/gen_replace——她的非战斗牌随机替换成战斗牌 |
...
- **惊鸿之舞（10020208）**：raw 效果清单悬挂空项待补，不录入（不知火仅 7 卡）。
- **日霭相织（10020221，协战主牌）及烟影（10020451）/煦日（10020551）**：raw 无效果文本，不录入。
- **铃鹿御前**：未加入（raw 未给出数据）。
