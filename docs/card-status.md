...
| 21 狂歌豪情 | ❌ | 协战主牌（茨木&酒吞，id 10010321）：酒吞侧子卡"醉酒当歌"未定义，主牌不入库；茨木侧子卡地狱豪焰已入库（见下行） |
...
（姑获鸟 8 张卡未引入，不可构筑——yaml 标 wip: true，构筑池过滤 available_shikigami 已排除；仅基础数据入库——刃影叠岚羁绊的搭档。）
...
（酒吞童子 8 张卡未引入，WIP——yaml 标 wip: true，构筑池过滤已排除；仅基础数据入库——地狱豪焰羁绊的搭档。）
