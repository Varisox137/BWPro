...
进场顺序语义（2026-08 落地，rules.md 第十六/十七章）：变形/还原均为再进场——`entry_order` = 本队 max+1（排本队最后）；回合开始倒计时批次按 entry_order 升序依次处理；还原时快照携带的剩余倒计时优先保留、不被能力进场重置初值。戏谑套索机制已存在无需改。关联点 6 条待确认见 questions.md。
