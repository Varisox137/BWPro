...
| 02 吾即正义 | ✅ | generate 谓词扩展（max_level=source / exclude_self）；计数 10 张法术后经 add_mod require 置位 transformed，改用 alt_effects（消灭所有敌方式神）；[瞬发] 变为后保留 |
...
| 卡牌 | 状态 | 备注 |
| --- | --- | --- |
| 基础能力 | ❌ | 式神级倒计时 + 破甲机制（破甲未实现） |
| 01-08 | ❌ | 破甲、[吸血]、伤害/破甲相互转化等新机制 |
