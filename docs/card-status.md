...
| 08 无我 | ✅ | |
| 21 森佑灵矢 | ❌ | 协战主牌（白狼&萤草，id 10010121；协战机制已实现，主牌待萤草 100127 引入） |
| 51 灵矢贯虹 | ⚠️ | 协战子选项（白狼侧战斗牌）：鼓舞消耗转化为本牌加成已实现（维护者答复 10）；「法术牌强化力量加成」与羁绊 1（萤草形态进场）两步缺机制/式神（见出入 10），主牌待萤草引入 |
...
| 03 大合奏 | ✅ | replay_countdown：按 countdown_history 首次出现顺序重放妖琴师生效过的基础/觉醒倒计时块（_countdown_block_for 按来源 id 找回块；维护者答复的顺序语义） |
...
| 06 觉醒·以津真天 | ✅ | 觉醒倒计时 initial 1（来源=觉醒牌 id）；"黄金羽可以敌方式神为目标"由黄金羽的使用方式表达（见出入 5） |
...
| 51 黄金羽 | ✅ | 衍生 token 法术（不可构筑）；基础效果固定打敌方牌手，觉醒后狙击走 methods |
...
| 51 蚀刃毒羽 | ✅ | 协战子选项（鸩侧战斗牌）：temp_grants 挂 on_damage，数量读 payload fragile（破甲受伤即消耗，见出入 9）；羁绊=以津真天倒计时-2 |
...
5. **黄金羽的觉醒目标扩展**："觉醒后黄金羽可以以敌方式神为目标"表达为黄金羽的使用方式
   （PlayMethod requires_awaken 门控 + choose 敌方式神目标）；基础效果仍固定打敌方牌手
   （未觉醒时照常可用，觉醒后二选一）。
...
7. **大合奏**：卡面"基础能力每生效过一种"按维护者答复实现——本局妖琴师的基础/觉醒倒计时
   ...
   不同对局生效顺序不同则大合奏顺序随之不同。
8. **觉醒·神乐歌的增益**："获得1力量与1生命"按临时修正实现（气绝清除；与 buff_power/
   buff_health 默认语义一致），非永久修正。
9. **蚀刃毒羽"相同数量的破甲"**：破甲在伤害结算的护甲计算批次被消耗（受伤即移除），
   on_damage 时目标已无破甲——数量读伤害管线记账的 payload `fragile`（本次消耗的破甲数），
   净效果为攻击后破甲量不变（消耗多少补回多少）。
10. **灵矢贯虹**：两步未实现——"本次攻击白狼获得当前自身法术牌强化效果的力量加成"
    （白狼法术牌强化机制尚不存在）与羁绊 1"攻击前触发萤草当前形态进场效果"（萤草 100127
    未引入）；羁绊 2 按维护者答复 10 实现（己方全部鼓舞战力/护甲作为本战斗牌加成赋予
    白狼并消耗；鼓舞关键字当前卡池不存在，落地后在 consume_assault_boosts 扩展）。
