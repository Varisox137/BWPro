...
# 04 沧海刀鸣（20200928，wip 骨架）

10 式神全部以 wip 骨架入库（式神 yaml 标 `wip: true`，不进构筑可选池与测试卡组）：仅基础数据+卡面原文，效果一律未实现（空 steps，打出无效果）。

| 式神 | 派系 | raw 数据 | 备注 |
|------|------|----------|------|
| 01 人面树（100401） | 紫岩 | 完整 | 8 卡（含幻境 凋零之森 耐久20、觉醒 +0/+2）+ 99 诅咒之木形态衍生实体（1/6）已按 raw 录入 |
| 02 跳跳哥哥（100402） | 红莲 | 仅卡名 | 8 卡默认骨架（spell/1级/R，text 空）+ 51 棺葬协战子卡骨架 |
| 03 樱花妖（100403） | 紫岩 | 完整 | 8 卡（含觉醒 +1/+1）已按 raw 录入 + 51 落英缤纷协战子卡骨架 |
| 04 三目（100404） | 紫岩 | 完整 | 8 卡（含觉醒 +2/+2）+ 衍生牌 51-66（紧急委托×4/今日委托×7/线索×5）已按 raw 录入；委托机制未实现 |
...
- 协战牌补齐：海国共主 10020721 本体与鸮羽共鸣 10030721 本体入库（副侧到位），子选项 琼玉镇海 10040851 / 鸮鸣 10040551 / 棺葬 10040251 / 落英缤纷 10040351 建骨架——四者 raw 均无文本（标未加入），text 留空待补；花骸缚骨/海潮深渊本体仍不建（raw 无荒骷髅/蟹姬条目）；繁花似锦（桃花妖&樱花妖）本体 raw 仅一行注记、无子卡定义，未建。
- raw 疑点（灾厄之花文本内层引号不闭合、诅咒之木注记"毁临"疑似笔误、委托牌无稀有度等）待确认见 questions.md。
...
- 已确定的协战牌 id（2026-07，card_data_raw 更新后）：
  ...
  - 繁花似锦（桃花妖 100119 & 樱花妖 100403）：raw 仅一行注记（无选择项文本/子卡名），主牌未建；樱花妖侧子卡 落英缤纷 = 10040351 已随 04 包建骨架（text 待 raw）
