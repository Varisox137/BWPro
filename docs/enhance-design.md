...
>
> **落地状态（2026-08 核对）**：第十节实施范围已全部落地（战斗上下文对象化与
> `launch_attack`、形态能力与实例写回、triggers/装配管线、CLI 修饰状态显示）；
> 实现以代码为准——schema 草案的 `monitors` 字段未采用，读取/装配两主通道由
> `conditional_keywords` / `conditional_mods` 承担（见 docs/terminology.md「实时监测」行）。
