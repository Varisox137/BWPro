...
2. **造成/受到伤害开始时**（即时时机）：（多种能力）。【破魔符/破魔箭已实现（裁决(4)）：受伤者式神带 `crit_pierce_mark` 时，本次伤害事件在本批次置位获得[暴击][贯通]属性（`_DamageEvent.crit_pierce` + piercing）——被贴标记者的反击伤害受伤者是攻击方（无标记），天然对防守侧不生效；效果伤害命中带标记者同样获得】
3. **贯通修正**（本次伤害不是反击战斗伤害、伤害原因具有"贯通"、且受伤者是式神时；伤害原因是否具有贯通见 terminology.md「贯通」——式神持有的贯通仅传导至其战斗伤害与基础/觉醒/形态能力伤害，不传导至其卡牌效果伤害。**例外**：经战斗上下文登记的反击贯通（counter_piercing op，战斗牌效果授予、绑定本次战斗）可使该战斗中被攻击方的反击伤害具有贯通，同样走本批次；破魔符事件属性（crit_pierce）同样放行反击事件的贯通修正——裁决(4)）：
...
7. 此时若气绝者没有（因插入结算）被标记为"已气绝"，则视为本次气绝事件将其击杀（影响部分卡牌的"增强"效果）。【**计入击杀时机**：位于"气绝前1"与"气绝后"延时时机之间（裁决(1)）——击杀账本已实现（`_account_kill`：来源方 `kill_total`/`kill_by` 分桶 + 灵咒击杀加成 + 委托 enemy_defeat 计数，规则评审⑩）；"将气绝时变棺材"延时变形生成晚于本时机（test_coffin_delayed_transform_after_kill_ledger 钉死次序）。击杀标记本身与"增强"机制一同引入】
...
10. **气绝后/消灭后**（延时时机）：各类能力（裁决(9)：不再按优先度细分）。跳跳哥哥各"将气绝时变为棺材"效果（不弃/罡身阵/棺封/棺葬）实际为"气绝前1 触发、本时机延时生成变形事件"，两时机之间为"计入击杀"（step 7，裁决(1)）（实现侧：`check_defeated` 在形态消灭前捕获 ext 旗标/形态 tag，气绝结算与击杀账本完成后把 `to_coffin` 追加为本气绝事件队列的收尾待结算项；棺葬经 `on_shikigami_defeated` 能力块同层）；棺击的增益效果触发于任一式神气绝事件的本时机。
...
- **破甲双倍** `double_damage_vs_fragile`（义道"对有破甲的式神造成双倍伤害"）：战斗牌专用提取步——按**[暴击]时机**（伤害事件"扣减生命前2"挂点，`Game._damage_double_modifiers`）：仅本张战斗牌发起的战斗内、攻击者本人、kind=combat（反击不翻倍）、victim=持破甲式神才 ×2；嵌套/插入战斗按战斗 id 精确匹配不继承（维护者定案）。[暴击]关键字本体（critical）已落地（破魔符/破魔箭 crit_pierce_mark 通道，见第三十六章与 terminology「暴击」条）。
...
- **破魔符标记与[暴击]**：ext["crit_pierce_mark"]（式神宿主，CLEAR_ANY_TURN_
  START）——"对其造成伤害的式神或卡牌获得[暴击][贯通]"（裁决(4)）：伤害事件
  "造成/受到伤害开始时"按**受伤者**标记置位事件属性（`_DamageEvent.crit_pierce`
  + piercing）——被贴标记者的反击伤害受伤者是攻击方（无标记），天然不生效；
  效果伤害命中带标记者同样获得（[暴击] 按关键字语义仍限攻击事件翻倍）；
  不可叠加（再贴幂等）。**[暴击]关键字本体**（critical）：攻击事件
  （kind=combat，反击不翻倍）伤害 ×2，挂"扣减生命前2"（义道同锚点叠乘）；
  限原始事件（start="full"——贯通溢出/转移衍生的新事件不再二次翻倍）。
  **[暴击]×[贯通]次序**（裁决(3)）：贯通的额外护甲计算 → 提前执行"扣减生命前"
  系列（含翻倍）→ 贯通分配（**翻倍后口径**）；溢出给牌手的伤害从"护甲计算
  前2"起走常规流程（跳过伤害开始时/贯通修正/翻倍修饰，扣减生命前系列不重复
  执行），不再二次翻倍。[暴击]/[贯通]/[吸血]均不叠加生效（可多来源临时共存）。
