...
| 来源个体排除 | `exclude_self` / `exclude_chosen`（TargetSpec 扩展键） | exclude_self：resolve 时按 ctx.source 排除效果来源个体（定案(0)："其他式神"= 除效果来源式神个体外的所有双方式神——镜像对局含敌方同名；与 exclude_shikigami 按数据 id 排除相别；0928 起樱吹雪改敌/己分池不再使用，键保留）；exclude_chosen：排除卡牌选择目标（增殖"使其他结附'蛊蚀'的敌方式神"——主目标不重复结算） | ✅ |
