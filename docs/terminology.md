...
| 行动账本 | ext["last_acted"] / context 目标 key `last_acted` | 控制者本回合最后一个行动（主动出击/使用专属牌——协战牌该式神侧子选项也算）的己方式神座次（CLEAR_OWN_TURN_START；行动后才气绝账本仍保留，读取处 in_play 门控；薰 棺击/觉醒·薰） | ✅ |
| 灵咒禁伤 | ext["no_damage_vs_inv"] | 本回合持有者对结附指定灵咒（值=灵咒名，不限来源）的式神造成的伤害无效（伤害管线早期终止、一切伤害类型——干扰投掷，bump_ext 写入，CLEAR_ANY_TURN_START） | ✅ |
...
| 灵咒条件键 | `invocation_on_field` / `holder_has_invocation` / `victim_has_invocation`（`_has_invocation` 后缀）/ `holder_countdown` / `shikigami_countdown_free`（条件运算符） | invocation_on_field=场上有己方式神结附该灵咒（麓鸣·穿[增强]，conditional_keywords 同名算子=其条件[瞬发]）；holder_has_invocation=能力持有者结附（棺击"持有'迟钝'期间"门控）；_has_invocation 通用后缀=事件中该前缀 Ref 所指式神结附（干扰投掷响应）；holder_countdown=持有者有/无[倒计时]（释煞阵）；shikigami_countdown_free=控制者式神（按 id）无[倒计时]（觉醒·跳跳哥哥 play_condition，未出战/气绝视为可用） | ✅ |
...
| 来源个体排除 | `exclude_self`（TargetSpec 扩展键） | resolve 时按 ctx.source 排除效果来源个体（定案(0)："其他式神"= 除效果来源式神个体外的所有双方式神——镜像对局含敌方同名；樱吹雪"其他所有式神"；与 exclude_shikigami 按数据 id 排除相别） | ✅ |
| 非召唤物过滤 | `not_summon`（TargetSpec 扩展键） | 仅非召唤物式神（定案(12)：选择目标合法性即排除召唤物——不弃"使一个非召唤物式神"；牌手目标被滤除） | ✅ |
| 行动前 | `on_before_card_played`（事件） | 即时时机（"主动使用牌"流程：确定牌/方式/目标与鬼火消耗之后、使用事件结算前——rules.md 第十三章）；仅主动使用路径发出（响应/凭空自动使用不发）；payload 同 on_card_played 核心字段 + `actor`=本牌所属式神 Ref（中立牌 None）；觉醒·薰用牌侧挂点（定案(11)，出击侧=on_before_assault 优先级 1） | ✅ |
...
| 随机取样复用 / 气绝纳入 | `memo` / `include_defeated`（TargetSpec 键） | memo=<key>：kind=all+random:n+memo 时同块后续步骤复用首次取样 refs（惊鸿之舞分支 13 力量与[贯通]落同一式神、分支 14 力量与生命落同一组——维护者定案）；include_defeated=true：friendly/enemy/any_shikigami 池纳入未离场气绝式神（惊鸿之舞分支 14"随机两名己方式神无论是否气绝"；any_shikigami 扩展=樱吹雪"其他所有式神"双侧口径）；`include_player`=true：friendly_injured 池追加受伤的己方牌手（"己方受伤角色"含牌手——答复(4)，落英缤纷羁绊） | ✅ |
