...
| 暴击 | `critical`（关键字）/ ext["crit_pierce_mark"] / ext["defeat_on_damage"] | [暴击]：攻击事件（kind=combat，反击不翻倍）来源式神持有时战斗伤害 ×2，挂扣减生命前（义道同锚点叠乘）；限原始事件（start="full"——贯通溢出/转移衍生事件不再二次翻倍，溢出量按翻倍前口径）。crit_pierce_mark（破魔符，式神宿主半回合）：实际为"对其攻击的式神在该次战斗中获得[暴击][贯通]"（裁决(11)——战斗开始按被攻击者判定授予、战斗作用域、不可叠加幂等）。defeat_on_damage（驱魔符，式神宿主半回合）：本回合"受到伤害时使其气绝"（伤害管线扣减生命后入气绝队列，同必杀通道） | ✅ |
