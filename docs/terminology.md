...
| 使用方式 | `play_method` / `PlayMethod` / `Game.usage_options` | 多择子选项；仅保留核心方式、参数可变（`param`，如爆能{2}）。扩展字段（不夜之火批次）：爆能英文=burst；`energy_cost`（int=爆能N 额外支付 N 能量 / "all"=爆能X 消耗全部能量、0 能量不可选；方式 `effects` 追加到基础 effects 后，多档独立支付累计）、`keywords`（以该方式使用时临时授予卡牌关键字——森之力爆能档得[瞬发]）。**手牌使用方式枚举（裁决(2)）**：`Game.usage_options(pi, card)` 返回该牌当前全部使用方式（常规/各 PlayMethod/协战子选项）+ 各自合法性/原因/鬼火与能量消耗（`_usage_method_error` 与出牌校验同口径、不变异状态）；CLI 热坐/联机手牌"使用方式"列统一渲染（`format_hand_lines`，调度阶段与敌方已展示手牌不渲染）；未来蓄力/赐能/起源等扩展方式在枚举侧追加条目 | ✅ |
...
| 能力伪关键字 | `power_if_field` / `power_per_field` / `power_if_shield` / `power_equal_shield` / `power_eq_health` / `heal_defeated_countdown` / `damage_defeated_countdown`（schema `ABILITY_PSEUDO_KEYWORDS`） | 属"式神能力"而非卡牌/实体关键字：写在式神基础能力 keywords 或觉醒牌 keywords 中，觉醒时引擎把基础式神的伪关键字移除、改为授予觉醒牌（perm 类别）——power_if_field 有幻境 +1 力量（泷夜叉姬）/ power_per_field 每幻境 +1 力量（觉醒·泷夜叉姬）/ power_if_shield 有护甲 +1 力量（久次良）/ power_equal_shield 力量=护甲（觉醒·久次良）/ power_eq_health 力量=当前生命（觉醒·人面树，覆写口径同 power_equal_shield——光环类效果，一次性力量增减益如萤草闪烁/封魔符不影响，定案(1)）/ heal_defeated_countdown 可对**己方**气绝式神恢复生命、改为气绝倒计时 -1（樱花妖基础/觉醒共用）/ damage_defeated_countdown 可对**敌方**气绝式神造成伤害、改为气绝倒计时 +1（觉醒·樱花妖）；两通道均按**结算时能力在场**判定（来源气绝/离场即不转化——维护者答复(3)）；身材类读取时求值（stat_aura 通道，实时光环动态跟随——定案(1)）、grant_keyword scope=turn 可临时授予（白骨之盾）；原 `field_stack`/`field_ability_stack` 作废删除（辉夜姬叠加改 field_merge 动作通道，定案(6)） | ✅ |
...
| 延迟能力 | `delayed` / `delay_grant`（动作） | 绑定式神的一次性延迟能力（会）：条目 {block, chosen, uses, secret, scope, watch}，事件匹配时先触发后执行、收集即消耗；气绝清除（变形离场保留）；scope="turn" 时己方回合开始清除（魔音扰心类）、scope="play" 时该次出牌结算结束清除（黑羽之刃类"本次使用期间"）、scope="battle" 时登记来源（watch）下一次作为攻击者的战斗结束即过期清除（麓鸣·轰"本次攻击后"，裁决(10)）；secret=True 时选择目标对敌方保密（联机脱敏抹除对手视角的 chosen） | ✅ |
...
| 灵咒 | `invocation` / `InvocationDef` | 结附于式神或卡牌上的具名效果实体（rules.md 第三十二章；04 沧海刀鸣批次落地实卡：迟钝/鸮之守护/八尺琼曲玉）：**定义已 yaml 化**——来源式神目录 `invocations.yaml`（loader 特判收集，versions 快照同卡牌管线按日期解析，顶层无 id，name 全局唯一）；测试亦可直接注入（version=0 恒可用）。字段 name/unique/power/health（效果类身材增减益=**类光环层**：结附时刻快照入条目、eff_power/max_health 读取时实时合计，不借 temp 通道——reset_stats 清除后立即继续生效（评审④定案）、移除即失效无双重扣减）/abilities（能力类触发块，进场序号=结附时刻 ability_seq，随 `_collect_abilities` 收集；含倒计时块时注册持有者唯一倒计时（回合开始批次归灵咒车道、晚于非灵咒扣减——定案(6)），`once` 块归零时**先移除灵咒再结算归零块**（定案(1) 移除眩晕早于攻击）并移除全部同名条目（定案(9)）——迟钝）/keywords（结附期间授予持有者，移除按实例撤销；"stun" 特判为眩晕条目 kind="invocation"、不过期）/draw_trigger（结附卡牌的"抽到触发"块）/text；运行时条目 `ShikigamiState.invocations` / `CardInstance.invocations`（{"name","player"=来源所属牌手,"source","uid"=条目身份,"power","health","bonus"=数值增强层,"mod_power"/"mod_health"=inv_mod 修饰层,"ability_seq","keywords","cd_block"}） | ✅ |
| 结附（灵咒） | `attach_invocation`（动作）/ `Game.attach_invocation` | name 指定灵咒；targets=式神结附 / uid=卡牌结附（targets 忽略）；`grant_keywords` 对结附条目追加关键字（无尽剑狱"结附期间持续[眩晕]"）；流程：结附（快照/序号/keywords/倒计时注册）→唯一性移除（bonus 继承）→inv_mod 重算→`on_invocation_attached`（延时）；气绝/离场经 `_detach_invocations` 移除（增强随之重置） | ✅ |
| [唯一] / [式神唯一] | `unique` / `shikigami_unique`（InvocationDef.unique） | 灵咒唯一性（结附之后移除，新结附自身保留；同源=来源所属牌手相同）：unique=一方全场（式神+手牌/牌库中的卡牌）同源同名移除；shikigami_unique=仅该式神上同源同名移除（卡牌结附不生效）；none=不唯一。**分侧定案（2026-08 裁决）：两级均相对一方牌手，双方同名可共存（含同一式神）**；同源同名再结附时新条目**继承**旧条目 bonus（取较大者）；持有方 ext["inv_override"][名]["unique"] 可覆写级别（祈愿之翼） | ✅ |
| 抽到触发 | `draw_trigger`（InvocationDef）/ `_proc_invocations_on_move` | 结附卡牌的灵咒在抽牌动作入手（reason="draw"，deck→hand）时入队延时结算（控制者=来源所属牌手）并移除；非抽牌入手静默移除；爆牌先触发再转墓地；调度换出直接移除不生效；多张抽牌顺序 = **倒序**（后发先至定案：第 2 张的移动+灵咒先结算，后抽的牌先入手、hand_seq 更小——严格递归 + 抽牌事件单元 drain） | ✅ |
...
| 灵咒结附后 | `on_invocation_attached`（事件） | 延时时机；payload {player=来源所属牌手, target: Ref\|None, **uid=灵咒条目 uid**（2026-08 语义变化：原为卡牌 uid/None）, invocation, source} | ✅ |
| 灵咒数值增强 | `bonus`（灵咒条目键）/ `inv_bonus_on_kill`（动作） | "效果+1"挂灵咒实体条目：灵咒层数值=快照+bonus+mod；同源同名再结附继承（取较大）、气绝/离场移除重置。inv_bonus_on_kill 登记牌手级击杀加成规则（ext["inv_bonus_on_kill"] 条目 {"inv","add"} 可叠加、CLEAR_NEVER——觉醒·大岳丸）：check_defeated 击杀账本段触发，击杀者身上该灵咒 bonus += add 并重算修饰层 | ✅ |
| 灵咒持有方修饰 | `inv_mod`（动作）/ ext["inv_mod"] / `_refresh_invocation_mods` | 牌手级灵咒数值修饰表：条目 {"name","shikigami"?=持有者限定,"add","mult"}，对持有方全部同名灵咒条目（不限结附来源敌我）eff=(快照+bonus)×mult+add；scope="ability" 条目随来源式神能力离场清除（`_clear_ability_card_auras`）、进场经 on_ability_enter 块重新登记（大岳丸基础/觉醒"结附于大岳丸时效果+1"） | ✅ |
| 灵咒结附覆写 | ext["inv_override"] | 持有方牌手覆写表 {灵咒名: {"unique"=级别覆写, "attach_all_friendly"=改己方全体在场式神结附}}，attach_invocation/唯一性判定读取（祈愿之翼"失去[唯一]但效果不能叠加+改为全体结附"；递归不再读覆写防循环） | ✅ |
| 伤害减免 | `reduce_damage`（动作） | 在伤害时点批次改写事件中可变伤害对象的数值 -amount（只减、下限 0——boost_damage 镜像；鸮之守护"受到伤害时减少1点"挂 on_damage_start + {victim_shikigami: self}） | ✅ |
| 改为结附 | `replace_action:<灵咒名>`（能力伪关键字） | 携带者出击或使用战斗牌时改为结附指定灵咒（跳跳哥哥）：动作分派处短路（不进战斗流程），鬼火/瞬发/出击次数照常消耗；带冒号参数按前缀归入能力伪关键字换绑集合（`engine._replace_action_invocation`） | ✅ |
| 棺材 | `to_coffin` / `coffin_revive`（动作）/ `coffin_origin` / ext["coffin_on_defeat"] | 棺材占位实体（kind=transform，与变形框架隔离；coffin_origin=原式神"气绝结算完成后"快照=正常复活 baseline——占位+普通复活非还原）：进入通道=to_coffin op（目标须已气绝非召唤物；keep_combat 时按 ext["defeated_in_combat"] 记账进战斗区）与气绝旗标（ext["coffin_on_defeat"] 不弃/形态 tags `coffin_on_defeat:<id>` 罡身阵——旗标路径的替换在"气绝后"延时时机生成，裁决(9)）；倒计时归零 coffin_revive 移除棺材走 `_revive`、跳跳家族（def 扩展字段 coffin_assault）立刻攻击且本次战斗[不屈]；**棺材被击杀（定案(8)）：还原原式神保持气绝、会有气绝事件且计入击杀账本**（含委托 enemy_defeat；入棺路径 to_coffin 维持原口径——其气绝事件/账本已按原式神结算过一次）；棺材被击破的气绝事件携带 `from_coffin=True` 判别字段（棺葬以 `from_coffin_not` 门控不再触发变棺） | ✅ |
| 攻击账本 | ext["last_attacker"] / context 目标 key `last_attacker` | 控制者本回合最后一个攻击（主动出击/战斗牌——_resolve_combat origin="assault"/"card" 单点记账，用牌与效果发起的攻击不记）的己方式神座次（CLEAR_OWN_TURN_START；攻击后才气绝账本仍保留，读取处 in_play 门控；薰基础能力/棺击账本读取） | ✅ |
| 灵咒禁伤 | ext["no_damage_vs_inv"] | 结附**己方牌手**的一回合效果（干扰投掷，定案(7)）：牌手 ext append 列表，条目 {"value": 灵咒名, "ref": [pi, 座次]} 限定被施 debuff 的式神——该式神气绝/复活不丢失，其对结附指定灵咒（不限来源）的式神造成的伤害无效（伤害管线早期终止、一切伤害类型；bump_ext host=controller+append+with_ref 写入，CLEAR_ANY_TURN_START） | ✅ |
| 灵咒光环 | stat_aura `kind="friendly_invocation"` / ext["inv_aura_kw"] | 己方在场且结附指定灵咒（name 参数，不限来源）的式神 +power/+health（鸮之利爪）；keywords 参数部分经 `_reconcile_invocation_aura_keywords` 读取时求值持续授予（多退少补，inv_aura_kw 记账；鸮之警惕[帷幕]/鸮之庇佑[不屈]，continuous 类授予不屈不被消耗移除） | ✅ |
| 灵咒条件键 | `invocation_on_field` / `holder_has_invocation` / `victim_has_invocation` / `attacker_has_invocation`（`_has_invocation` 后缀）/ `shikigami_countdown_free`（条件运算符） | invocation_on_field=场上有己方式神结附该灵咒（麓鸣·穿[增强]，conditional_keywords 同名算子=其条件[瞬发]）；holder_has_invocation=能力持有者结附（棺击"持有'迟钝'期间"门控）；_has_invocation 通用后缀=事件中该前缀 Ref 所指式神结附（干扰投掷响应 victim_ 实例；attacker_has_invocation=攻击者本人结附——鸮鸣"己方具'鸮之守护'的式神发起攻击"、琼玉镇海牌手级监听）；shikigami_countdown_free=控制者式神（按 id）无[倒计时]（觉醒·跳跳哥哥 play_condition，未出战/气绝视为可用） | ✅ |
| 灵咒目标过滤 | `has_invocation`（TargetSpec 扩展键） | 结附指定灵咒的式神（牌手目标被滤除）；值=灵咒名或 {"name","same_source": true}（同源限定=结附来源所属牌手为控制者——决意"你结附'鸮之守护'的式神"） | ✅ |
| 符咒 / 爆能转化 | `subtype: talisman` / card_aura `grant_method` 参数 / card_aura `subtype` 参数 | 符咒子类型（御馔津 驱魔符/封魔符/破魔符；普通子类型、无排他归属）；grant_method=给命中牌挂使用方式（方式模板 dict：id/energy_cost/card_type/target/power/shield/effects/text——御馔津基础/觉醒"符咒牌都具有[爆能3]：转化为战斗牌且改为攻击任一敌方式神"）：方式按生效 card_type 路由（combat=战斗流程，身材取方式 power/shield、方式 effects 整体替换原 effects 缺省=空块）、耗能量+鬼火、须主动选择合法目标（裁决(9)）；爆能使用时仍是符咒牌子类型（subtype 不随转化丢失，裁决(9)）；scope=ability 随能力离场/觉醒换绑移除。card_aura subtype 参数=仅命中该子类型的牌（狐狩界给符咒牌授[瞬发]） | ✅ |
| 符咒账本 | ext["talisman_ledger"] / `cast_ledger`（动作） | 奉祝之愿账本（裁决(10)）：牌手级 CLEAR_NEVER，本局使用过的符咒牌 id 列表（按使用顺序、去重、至多 3 种，`_account_card_played` 记账；爆能转化的战斗牌使用按原符咒 card_id 记账）；cast_ledger=按序对目标依次自动使用所记录的每种符咒（奉祝之愿出击对被攻击式神；目标无形态也照常使用驱魔符） | ✅ |
| 爆能消耗折扣 | `burst_discount`（幻境实体关键字） | 你的所有[爆能]消耗 -1/个（狐狩界；可叠加——裁决(11)，能量消耗读取处按在场幻境数求和） | ✅ |
| 暴击 | `critical`（关键字）/ ext["crit_pierce_mark"] / `_DamageEvent.crit_pierce` / ext["defeat_on_damage"] | [暴击]：攻击事件（kind=combat，反击不翻倍）来源式神持有 critical（或事件经 crit_pierce 置位）时战斗伤害 ×2，挂"扣减生命前2"（义道同锚点叠乘）；多来源共存不叠加（布尔判定，裁决(3)）；限原始事件（start="full"——贯通溢出/转移衍生事件不再二次翻倍，贯通分配按**翻倍后**口径：贯通修正先额外护甲计算→提前执行扣减生命前系列（含翻倍）→贯通分配，裁决(3)）。crit_pierce_mark（破魔符/破魔箭，式神宿主半回合，再贴幂等）："对其造成伤害的式神或卡牌获得[暴击][贯通]"——伤害事件"造成/受到伤害开始时"按受伤者标记置位事件属性（裁决(4)；反击伤害受伤者是攻击方而天然不生效；效果伤害命中带标记者同样获得，[暴击]仍按关键字语义限攻击事件翻倍）。defeat_on_damage（驱魔符，式神宿主半回合）：本回合"受到伤害时使其气绝"（伤害管线扣减生命后入气绝队列，同必杀通道） | ✅ |
| 蛊蚀 / 结附增幅 / 存量增减 | `invocations.yaml` 蛊蚀 / `inv_attach_bonus` / `inv_count_mod`（动作）/ ext["inv_attach_bonus"] | 蛊蚀=unique=none 可叠加灵咒（power/health -1/-1，一次性赋予的持续性光环减益、来源可为双方——裁决(12)）；**致死检查** `_invocation_lethality`：结附/存量增减改变生命上限后立即检查，上限（含 inv_mod 修饰层）≤0 即气绝（上限降低先钳当前生命、不发事件）。inv_attach_bonus 登记牌手级结附增幅规则表（条目 {"name","add"} 可叠加、CLEAR_NEVER——觉醒·巫蛊师）：attach_invocation 按来源牌手读取叠加，仅'结附'动作触发（裁决(13)）。inv_count_mod=灵咒存量增减（增殖）：按条目追加/移除，不发结附事件、不吃增幅、不做唯一性移除（限 unique=none），变化后重算修饰层并致死检查 | ✅ |
| 气绝灵咒快照 / 魔蛊毒爆转移 / 伤害改结附 | `victim_invocations`（on_shikigami_defeated payload）/ `victim_invocation`（条件键）/ `victim_invocation_count`（动态数值键）/ `inv_transfer_on_defeat` / `redirect_to_invocation`（动作）/ ext["inv_transfer_on_defeat"] | victim_invocations=气绝移除前灵咒名快照（按条目重复——灵咒随气绝先移除，活体读不到）：条件键 victim_invocation 存在性（无尽蛊）、动态数值 {"victim_invocation_count": 名, "per": n} 计数（食魂蛊"其上每有一个'蛊蚀'"）。inv_transfer_on_defeat 登记牌手级半回合转移（{"victim","inv"}，CLEAR_ANY_TURN_START——魔蛊毒爆，裁决13/14）：视作赋予登记目标敌方式神的一次性能力——其本回合气绝时由该敌方式神侧触发（来源牌手=气绝者所属方，不吃登记方 inv_attach_bonus 增幅、严格等量），将其上等量（移除前快照计数、不论来源敌我）灵咒一次性结附于随机另一名在场敌方式神（非选择目标、帷幕可结附、一次性消耗）。redirect_to_invocation（缚蝶蛊狱挂 on_after_shield）：伤害改为结附等量灵咒并终止伤害事件（裁决(15)，双方式神受影响） | ✅ |
| 力量等值过滤 | `power_eq`（TargetSpec 扩展键） | 力量恰等于 n（缚蝶蛊狱"消灭所有力量为0的式神"；牌手目标被滤除） | ✅ |
| 牌库灵咒（梦魇） | `attach_deck_invocation` / `redirect_deck_invocation`（动作）/ `on_invocation_drawn`（事件）/ ext["drew_invocation_turn"] / `enemy_drew_invocation`（条件键+conditional_keywords 算子）/ `{"deck_invocation_count": {"name","side"}}`（动态数值键）/ stat_aura `kind="enemy_deck_invocation"` | 灵咒结附于牌库中的牌（食梦貘'梦魇'，unique=none 多张共存、不重复标记同一张）：抽到触发（draw_trigger）结算时记惊梦账本（牌手宿主、任一回合开始清除）并发 on_invocation_drawn（延时，{player=抽牌者, card, invocation, source_player}——梦中低语/永眠之梦监听挂点）；非入手离库静默移除（move_card 通道）、回库保留。enemy_drew_invocation=对方本回合抽到过结附该灵咒的牌（惊梦[瞬发]）。deck_invocation_count=指定侧牌库标记牌计数（安魂回血，配 base/per）；enemy_deck_invocation 光环=敌方牌库每有一张标记牌持有者 +1/+1（支配者，读取时求值）；redirect_deck_invocation（支配者挂 on_after_shield priority 1）=受伤归零终止、随机移除敌方牌库 min(伤害,cap=3) 张标记牌入墓地（thoughts(1)：伤害>3 按 3、不足全移除、光环降身材致死检查），无标记牌空操作 | ✅ |
| 鬼斩 / 灵咒触发宣告 | `on_invocation_trigger`（事件，insert）/ `announce_invocation` / `choose_invocation` / `detach_invocation`（动作）/ `holder_in_combat`（条件键）/ `card_shikigami`（context 目标键）/ launch_attack `at="event"/"targets"` + `keywords`/`ignore_veil` 参数 / `virtual_combat` / `inv_trigger_echo` / `power_on_enemy_turn:N`（伪关键字） | 鬼切三鬼斩（髭切/友切/狮子之子，shikigami_unique 同名替换、不同名共存）：announce_invocation 发 on_invocation_trigger（即时时机——响应挂点，payload {player=持有者方, invocation, holder: Ref, target: Ref\|None}）；choose_invocation 选择结附（剔除已结附候选，>1 挂起 pending invocation_pick 作答 choice=灵咒名）；detach_invocation 逐条移除（鬼切"敌方回合结束时移除所有的鬼斩"）；holder_in_combat=持有者方战斗区座次==持有者或持 virtual_combat（复仇之刃准备区视同）；card_shikigami=事件所携牌所属式神在事件 player 方的在场座次（友切目标，未出战/中立牌为空）；友切定案(4)=任意方式任意位置使用的法术专属牌均触发（"使用前"统一发点 `_emit_before_card_play`：主动/响应/自动使用同事件），且攻击可无视帷幕（launch_attack ignore_veil——灵咒效果不受帷幕限制）；launch_attack at=event 取事件 shikigami 字段为战斗目标（髭切 insert 先攻）、at=targets 取本步解析目标、keywords 经 next_battle_keywords 挂账（影杀[直击]）；inv_trigger_echo=灵咒能力块收集两份（刀鸣之刃"额外复制一次"，响应名额按窗口+事件名去重不双响应）；power_on_enemy_turn:N=敌方回合 +N 力量（散华之刃，_refresh_stat_auras 伪关键字段前缀扫描） | ✅ |
| 响应覆盖块分派 / 反制 | `cdef.response` 分派改动（`_settle_response_card`）/ `counter_on_kill`（动作）/ `_counter_watches` / `_active_play_marker` | 带 response 覆盖块的响应牌（鬼斩响应三连两断/罗城门/影杀起含战斗牌）一律落墓地走覆盖块 steps、不走战斗/形态分派（响应战斗牌不发起战斗，攻击语义由覆盖块组合表达；存量 19 张覆盖块全是法术，后向兼容）；counter_on_kill（罗城门"若击杀式神，反制他使用的法术牌"）登记反制挂账 {"attacker": Ref}——check_defeated 钩子：击杀者匹配且有进行中的出牌（_active_play_marker=on_before_card_play marker）时置 nullified（魔音扰心同通道，牌照常进墓地），条目消耗、战斗终止点清除残留 | ✅ |
| 交互检索 | search_deck `choose=True` / pending `search_pick` | ❌ 已废弃（thoughts(6)：觉醒·鬼切定案为"选择并凭空生成"，改走 pick_generate 通道；search_pick 通道已拆除） | ❌ |
| 双选择目标 | `target2`（CardDef）/ `chosen_index`（TargetSpec 扩展键）/ `at_index`（launch_attack 参数） | 第二 choose 目标（麓鸣·灭）：出牌指令 cmd["target2"] 校验后 ctx.chosen=[主目标, 第二目标]；step 目标 {kind: choose, chosen_index: n} 按序取；launch_attack at=chosen + at_index 按序取攻击目标；热坐/联机 CLI 依次提示两目标（`client/cli.py prompt_target` 共用，联机 dict 透传、服务端校验）。**战斗牌效果 ctx 携带 chosen**（`_resolve_combat_card`——麓鸣·轰 delay_grant bind=chosen 通道） | ✅ |
| 选择目标排除 | `exclude_chosen`（TargetSpec 扩展键） | 排除卡牌选择目标（增殖"使其他结附'蛊蚀'的敌方式神"——主目标不重复结算）；原 exclude_self（来源个体排除）随樱吹雪 0928 改敌/己分池删除 | ✅ |
| 非召唤物过滤 | `not_summon`（TargetSpec 扩展键） | 仅非召唤物式神（定案(12)：选择目标合法性即排除召唤物——不弃"使一个非召唤物式神"；牌手目标被滤除） | ✅ |
...
| 战斗结束后 | `on_battle_end`（事件，延时队列时机） | 战斗正常完成（走完"战斗后"时机且未被替换）才由 `_resolve_combat` 收尾发出（裁决(10)，rules.md 第二章三节）：取消/过早终止（攻击者气绝/鸦羽疾走/攻击替换）不发；发点时本战斗攻击增益已核销——挂此时机的延时打击不继承增益（裁决(2) 影杀次序；麓鸣·轰"本次攻击后"挂点） | ✅ |
...
| 气绝前 1 / 气绝前 2 / 气绝后 | `on_before_defeat` / （`check_defeated` 内联）/ `on_shikigami_defeated` | 气绝事件时机分层（裁决(9)，rules.md 第七章）：**气绝前1**=即时时机 `on_before_defeat`（check_defeated 开头 emit）——大部分能力/响应挂此时机（射怪鸟事/破碎之音/不灭之火/不祥之刃/判官能力/英雄无畏后续等）；**气绝前2**=仅解除变形（check_defeated 内联快照还原）+ 替身（未引入）；**气绝后**=延时时机 `on_shikigami_defeated`，不再按优先度细分——跳跳哥哥"将气绝时变为棺材"（不弃/罡身阵/棺封/棺葬）= 气绝前1 触发、气绝后延时生成变形事件（check_defeated 捕获旗标/形态 tag 后把 to_coffin 追加为该气绝事件队列收尾待结算项）；棺击增益挂此时机 | ✅ |
...
| ext 计数 | `bump_ext`（动作） | 目标式神/牌手 ext[key] 累加：倒计时能力的"每触发一次 +1"以块内 step 表达（鸩 x=zhen_proc）；ext 不随气绝清除。参数扩展：value=直接写入旗标/名值；host="controller" 写入控制者牌手 ext（"结附己方牌手"类，干扰投掷定案(7)）；append=True 同键列表追加；with_ref=True 追加条目记 {"value", "ref": [player, 座次]}（牌手结附但限定具体式神身份） | ✅ |
| 每回合合计一次 | `turn_mark` / `turn_mark_not` / `turn_count_eq` | 标记存 `PlayerState.ext["turn_marks"]`，任一回合开始双方清除；能力条件以 {turn_mark_not: key} 求值（寂寥心象；标记须先于分支步骤，连锁事件被门控挡）。turn_mark count=True：按次累加为 int（"本回合第 N 次触发"计数通道——鸮鸣），以 {turn_count_eq: {key, count}} 恰等求值（收集阶段求值——同事件先结算的计数步对同批后匹配块不可见） | ✅ |
...
| 倒计时事件 | `on_countdown_proc` / `on_countdown_reduced`（事件） | proc=倒计时能力归零生效时（**即时**时机、先于归零块结算——烈/刚/斩类监听授予的增益/关键字赶上归零块发起的攻击；payload {shikigami: Ref, source, once}）；reduced=倒计时每次减少（**即时**时机——定案"倒计时减少触发效果都是即时时机"，觉醒·山风复制为特例、单卡覆盖 timing=queue 延时执行；payload {shikigami: Ref, original 原始减少量 / actual 实际=修正到剩余 / by_card 是否卡牌效果 / natural 是否回合开始批次自然减少}——势按 original 取值（定案(5)）；觉醒·山风共享 2026-08 二轮定案：范围=任何己方减少倒计时效果（含能力来源，`natural_not` 门控、自然减少不共享），无倒计时能力的未气绝式神被减少也发事件（actual=0、original=减少量、状态不变），复制延时界=引起该次减少的结算单元（`_Pending.horizon` + `Game._drain_horizon`，能力块完即复制、卡牌完才复制）；原始-实际之差累计写入块内暂存 `countdown_overkill`——突"过量效果"动态数值 {"memo": "countdown_overkill"}） | ✅ |
...
| 随机取样复用 / 气绝纳入 | `memo` / `include_defeated` / `include_defeated_kw`（TargetSpec 键） | memo=<key>：kind=all+random:n+memo 时同块后续步骤复用首次取样 refs（惊鸿之舞分支 13 力量与[贯通]落同一式神、分支 14 力量与生命落同一组——维护者定案）；include_defeated=true：friendly/enemy/any_shikigami 池纳入未离场气绝式神（惊鸿之舞分支 14"随机两名己方式神无论是否气绝"等无条件口径）；include_defeated_kw=true：樱花妖气绝入池门控（定案(2)——己方气绝 iff 在场 heal_defeated_countdown、敌方气绝 iff 在场 damage_defeated_countdown，choose 校验与 all/随机解析同口径、随机池每次重算；适用池另含 friendly_character/enemy_character/any_character）；`include_player`=true：friendly_injured 池追加受伤的己方牌手（"己方受伤角色"含牌手——答复(4)，落英缤纷羁绊） | ✅ |
...
| `drew_invocation_turn` | `PlayerState.ext` | 惊梦账本：本回合抽到过结附灵咒的牌的灵咒名列表（_proc_invocations_on_move 抽到触发时记账；任一回合开始清除——半回合作用域；enemy_drew_invocation 条件键/conditional_keywords 算子读取） |
...
| 委托条件账本 | `quest_counts`（`PlayerState` 字段）/ `quest_count_ge`（条件键）/ `deck_names`（`PlayerState` 字段） | 三目委托机制（rules.md 第三十三章）：牌手级 12 种 kind 计数（assault/draw/play/damage/effect_damage/attack/form_play/offdeck_play/enemy_defeat/revive/quest_used/round，记账点 `Game._quest_tick`/`_account_quest_damage`/`_start_turn`），跨区域有效（"在牌库也有效"）；{quest_count_ge: {kind, count}}=控制者账本达标（play_condition/Step 级条件通用）；play 不限主动/响应/自动（定案(3)）；offdeck_play 口径（定案(5)）=同名牌不在本局卡组（deck_names，构筑时今日委托替换后按牌名捕获）；round（定案(5)）=己方回合开始 +1（原 round_ge 全局轮数键作废删除）；CLI 手牌进度标签 `_quest_progress`；**多事多忙扩域**（tags `quest_enemy`）：行为类 kind 记账时对方在场形态含此标记则对方账本同计（定案(4)），归属类（enemy_defeat/revive/quest_used）不扩域 | ✅ |
| 委托视为达成 | `quest_done`（mods 键）/ `quest_complete`（动作）/ `quest_complete_pick`（pending_choice kind） | quest_complete{mode=choose\|random}：手牌中 tags 含 `urgent_quest` 且未标记的实例置 `mods["quest_done"]`——`_play_condition_met` 读取，[条件]使用前提视为满足（委托整理 choose 多张挂起交互 uid 作答 / 休暇 random；空候选空操作） | ✅ |
...
| 气绝转化参数 | `allow_defeated` / `repeat_on_kill` / `repeat_on_revive`（damage/heal 参数） | allow_defeated：气绝（未离场、等级≥1）式神目标放行并转化——heal 改为气绝倒计时 -1（减到 ≤0 立即复活）、damage 改为 +1（均不再视为治疗/伤害、不发事件）；卡牌文本自带"（无论是否气绝）"授权通道（不分侧向），与来源伪关键字 heal_defeated_countdown（仅己方目标）/damage_defeated_countdown（仅敌方目标）并存——伪关键字通道按结算时能力在场判定（答复(3)）；气绝目标不进伤害/治疗管线（未授权拦截空过）且不计 repeat 新击杀；repeat_on_kill/repeat_on_revive：次数参数（int）——本步造成新气绝/引发复活即对原目标列表整体重复，至多 n 次（樱吹雪"重复此效果一次"=1；0928 起伤害池=enemy_shikigami、治疗池=friendly_shikigami 分侧向，伤害段全部重复完才进治疗段；上限 10 次防死循环） | ✅ |
...
| 有形态目标过滤 | `has_form`（TargetSpec 过滤键） | 仅结附着形态的式神（牌手滤除——神木诅咒取对象口径）；`prefer_wounded`：候选中存在受伤（生命<上限）或气绝式神时收窄到该子集再交 random 均等取（晚樱之意"优先受伤或气绝式神"，气绝者入池需配 include_defeated_kw）；include_defeated/include_defeated_kw 同步纳入 choose 校验池（spec_pool_refs） | ✅ |
