...
| 交互检索 | search_deck `choose=True` / pending `search_pick` | 命中 >1 张不随机、挂起由控制者选 uid 置入手牌并洗牌（觉醒·鬼切"选择一张鬼切的战斗牌置入手牌"）；0/1 张不挂起、未命中不洗牌（花信风先例） | ✅ |
